...
- 保证生产环境采用**Linux centOS 7.x**

- 保证**web服务器**采用**Apache**或**Nginx**

- 保证 **Swoole**扩展版本大于等于 **4.6.7**

- 保证**RabbitMQ**版本Distro/Version为**el/7**；**erlang**版本大于23.2小于24及Distro/Version为**el/7**。
...
ORM实现了mysql增删改查常用的操作，避免书写有漏洞的sql、优化实体类结构，如防止sql注入，同时让业务易于理解、更改。
...
2、ORM非原生写法操作时，首先选择操作模式，写法为：$mysql_orm->model(''); 形参有四种，分别为：insert、delete、update、select，model()方法后的类方法顺序请按照文档顺序调用，如：$mysql_orm->model('select')->from('*,user')->where('sex=&$nan&')->order_by('id,desc')->limit('0,10')->query();
...
- 协程仅支持在server目录下服务器、多进程文件中创建，如在TCP服务器中创建，放置在匿名回调函数中：
  ...
  **//co::sleep()为模拟IO不阻塞执行，实际开发中IO等待会自动挂起该协程并运行其他协程中的业务实现异步。当多个协程进行IO操作时，请为每一个协程配置co::sleep()以避免发生同一时间片资源争抢而导致部分IO执行失败。**

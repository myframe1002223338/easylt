...
1.请参考model模型目录下的mysql_test.php文件中的代码进行创建新的model文件编写模型业务，所有实体类中对关系型数据库mysql可采用ORM（文档见下文）或原生写法（mysql连接变量为$mysql_conn），如果采用原生mysqli写法，入参参数请用mysqli_real_escape_string()函数进行转义并在sql中对参数添加单引号，防止sql注入；对非关系型数据库redis采用原生写法（redis连接变量为$redis）。

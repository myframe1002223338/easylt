#####                        EASYLT框架采用全新的MSVP架构，全名EasyLite，寓意为简单、轻松、优雅的开发项目。在倡导无代码、低代码的大环境下，无论是初学者还是资深开发者，EASYLT都可快速上手完成项目的业务需求及进阶开发。EASYLT是MVC/MVP架构的升级版，在原有的model层中增加server层，可实现业务代码内存常驻，用于多进程、协程的TCP、UDP、HTTP、WEBSOCKET协议服务及RPC服务搭建，可快速实现业务RESTful、RPC的API及覆盖web、小程序、APP等开发的各种应用场景。使用前请仔细阅读以下文档，除标题外，加粗字体部分为重点需要注意的地方，否则框架运行可能会出错。
...
1.请打开core/config目录，对所有配置文件进行相应的修改配置，如mysql连接参数等；另外请在config.php中配置更改常量ADDRESS为你的服务器公网IP 或 域名配置，请以http://或https://开头。

2.请将backup目录下的easylt_test.sql文件导入mysql的指定数据库中，用于测试框架完整度，可忽略。

3.下载框架后，在application目录下的model、model-server、presenter-logic、view目录中包含若干测试示例demo，可参考开发，使用框架开发项目时请删除，其他任何文件、目录均不可删除，如下：

- model模型目录下的server目录、.htaccess
- model-server服务目录下的server.ini
- presenter-logic逻辑目录下的.htaccess
- view视图目录下的.htaccess、start.php
...
2.点击"点击进入view视图mysql测试页面"，如果框架运行成功会出现：数据库连接成功！数据库读取值为10，logic逻辑处理后值为15，该步骤用于测试框架完整度，可忽略。



**application – model**

1.请参考model模型目录下的mysql_test.php文件中的代码进行创建新的model文件编写模型业务，所有实体类中对关系型数据库mysql操作可采用ORM（操作文档见下文）或原生写法（**mysql连接变量为$mysql_conn，采用mysqli面向过程写法**），原生写法入参参数建议用mysqli_real_escape_string()函数进行转义并在sql中对参数添加单引号，防止sql注入；对非关系型数据库redis采用原生写法（**redis连接变量为$redis**）。

**2.$query_result为请求的入参数据,数据类型为array，变量名不可更改，根据键值对取值。**
...
**4.model传递到对应的logic文件数据的变量名默认为$data，变量名可自行更改，如model复用include()其他model文件，请设置不同的model文件下的数据变量名以免变量名冲突。**

**5.model或logic文件中的数据出参回传变量命名必须为$response且格式如下：**

   **$response = ['code'=>200,'msg'=>'response success!','data'=>'返回数据'];**

**6.如果model没有对应的logic文件，请在model中返回数据出参回传，否则请在logic中返回数据出参回传。**

**7.model复用：mvp框架模式下的logic是为了model复用同时与控制器解耦，使得采用一个默认控制器，业务逻辑单元放置到logic中，便于更改业务同时不影响原有的model模型；如需复用model直接在model模型目录下创建文件并include()引入需要复用的model文件即可，同理如果没有对应的logic文件，请在model中返回数据出参回传，否则请在logic中返回数据出参回传，两种情况请参考框架中copy.php及copy.logic.php的示例。**

**PS：被复用的model文件不能直接数据出参回传，请默认都在logic文件中处理业务逻辑单元及数据出参回传。**
...
**1.view视图目录下仅支持创建后缀名为.php的文件，即使书写html、css、js，不支持.html等格式。**
...
1.model模型文件中的数据业务如交给logic逻辑文件中处理，则在logic中最终返回数据出参回传，否则请在model中返回数据出参回传，数据出参回传格式请参考以上application – model中说明。

**2.logic逻辑下的文件命名规范：model相应文件名拼接.logic，如model模型目录下的文件名为mysql_test.php，则logic逻辑目录下的相应文件名必须为mysql_test.logic.php。**
...
1.swoole是一个多进程、支持协程的客户端、服务器php扩展，支持TCP、UDP、HTTP，WEBSOCKET协议及RPC远程过程调用客户端、服务器的实现，EASYLT对其进行高度封装并融合于MVP框架，使开发变得更简单、可靠。PS：感谢韩天峰老师开发、分享的Swoole开源项目，重新定义PHP。

2.swoole创建的服务器脱离FPM、WEB服务器运行，可常驻内存，可实现多进程、多协程、异步非阻塞业务，以下将分别说明各个服务器、客户端创建过程。

3.服务运行模型有两种：即**view->model->model/server**与**view->model/server**

- **view->model->model/server：**view视图目录下的视图文件，通常为AJAX或curl调用model模型目录下的client客户端，model模型目录下的client客户端文件通过TCP等通信协议连接model/server服务器目录下的服务器文件；model/server服务器目录下的server服务器文件仅可通过CLI命令台进行启动。框架在model、model/server下已经创建了各个客户端、服务器的示例，仅供参考。
...
- **model-server服务下创建的TCP、UDP、HTTP、WEBSOCKET、RPC服务器仅支持redis、mysqli的原生DB操作，不支持mysql-ORM；除了WEBSOCKET服务器中的workerstart类方法中的回调函数、多进程创建的create类方法中的回调函数不需要引入数据库连接变量直接使用形参变量外，任何其他协议服务器的类方法中的回调函数操作数据库均需先引用数据库连接变量，mysql连接变量为$mysql_conn，redis连接变量为$redis，如global $mysql_conn；**

- **如果服务器是创建在多进程中，则使用闭包引入数据库连接变量，如：**
  ...
  ​         $tcp_serv->receive(function($request_data)use($mysql_conn,$redis){

  ​         **//通过闭包use($mysql_conn,$redis)引入数据库连接变量**
...
- 当接收到请求数据时【必须实现】
  ...
  //如需返回给客户端数据请return $var，如在$websocket_serv->workerstart(function(){});的匿名函数方法体中return $var返回给客户端数据，则这里不进行数据返回，如果没有业务实现调用该事件后方法体为空即可，receive、workerstart事件中仅可选择一个进行数据返回。

  //$websocket_serv->receive(function(){});的方法体中可实现一对一（单聊），一对多（群聊）业务，请在config_swoole.php中配置，默认为单聊模式。

  **ps:数据读操作时请勿使用mysqli_close()、$redis->close()，会导致无法持续从数据库中取数据推送给客户端，可以释放结果集。数据库写操作时一定要mysqli_close()、$redis->close()，否则内存常驻+websocket的默认定时任务会持续写入操作。**

  });

- 当worker进程启动时，仅可用于长连接主动推送一对一（单聊）数据给客户端【可选项】
  ...
  **ps:数据读操作时请勿使用mysqli_close()、$redis->close()，会导致无法持续从数据库中取数据推送给客户端，可以释放结果集。数据库写操作时一定要mysqli_close()、$redis->close()，否则内存常驻+websocket的默认定时任务会持续写入操作。**
  ...
  //$websocket_serv->workerstart(function(){});的方法体中仅可实现一对一（单聊）业务，可根据需求应用于其他长连接业务场景。通常实现单聊、群聊用以上$websocket_serv->workerstart(function(){});即可。
...
**15.创建多进程：**仅支持在server目录下服务器文件中创建，可参考框架内示例文件：process_test.php
...
- 创建多进程【必须实现】
  ...
  $process->create(function($process,$mysql_conn,$redis){**//创建第一个进程**
  ...
  ​    $process->push(mt_rand(1,9));//写入数据到消息队列中，使用消息队列需在config_swoole.php中开启多

  ​    进程消息队列；

  });

  $process->create(function($process,$mysql_conn,$redis){**//创建第二个进程**

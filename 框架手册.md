...
#####                        EASYLT框架采用全新的MSVP架构，全名EasyLite，寓意为简单、轻松、优雅的开发项目。在倡导无代码、低代码的大环境下，无论是初学者还是资深开发者，都可快速上手完成项目的业务需求及进阶开发。EASYLT从项目健壮性、应用层面出发，有针对性的选择三方开源扩展、中间件，如消息队列中间件封装了RabbitMQ，更多同类中间件，如Kafka，如果有开发需求可以自行实现，让项目开发更简易同时开发者不依赖框架。MSVP架构是MVC/MVP架构的升级版，在原有的model层中增加server层，可实现业务代码内存常驻，用于多进程、协程的TCP、UDP、HTTP、WEBSOCKET协议服务及RPC服务、RabbitMQ消息中间件服务搭建，可快速实现业务RESTful、RPC的API及覆盖web、小程序、APP等开发的各种应用场景。使用前请仔细阅读以下文档。
...
- model/server服务目录下的server.ini、connect.ini
...
2.请参考model模型目录下的mysql_test.php文件中的代码进行创建新的model文件编写模型业务，所有实体类中对关系型数据库mysql操作可采用ORM（操作文档见下文）或原生写法（**mysql连接变量为$mysql_conn，采用mysqli面向过程写法**），原生写法入参参数建议用mysqli_real_escape_string()函数进行转义并在sql中对参数添加单引号，防止sql注入；对非关系型数据库redis采用原生写法（**redis连接变量为$redis**）。

**3.$query_result为请求的入参数据,数据类型为array，变量名不可更改，根据键值对取值。**

**4.$query_result['param']为请求URL的路由param值,用于判断同一model文件下多接口业务分发对接，如：**

   **http://127.0.0.1/m/v/p/index/model/a/?name=easylt中$query_result['param'] = 'a';**

**5.model传递到对应的logic文件数据的变量名默认为$data，变量名可自行更改，如model复用include()其他model文件，请设置不同的model文件下的数据变量名以免变量名冲突。**

**6.model或logic文件中的数据出参回传变量命名必须为$response且格式如下：**

   **$response = ['code'=>200,'msg'=>'response success!','data'=>'返回数据'];**

**7.如果model没有对应的logic文件，请在model中返回数据出参回传，否则请在logic中返回数据出参回传。**

**8.model复用：msvp架构模式下的logic是为了model复用同时与控制器解耦，使得采用一个默认控制器，业务逻辑单元放置到logic中，便于更改业务同时不影响原有的model模型；如需复用model直接在model模型目录下创建文件并include()引入需要复用的model文件即可，同理如果没有对应的logic文件，请在model中返回数据出参回传，否则请在logic中返回数据出参回传，两种情况请参考框架中copy.php及copy.logic.php的示例。**

**PS：被复用的model文件不能直接数据出参回传，请默认都在logic文件中处理业务逻辑单元及数据出参回传。**

9.model/server：用于创建各种通信协议服务器，请参考以下**SWOOLE、RabbitMQ框架操作**。
...
**15.创建多进程：**仅支持在server目录下服务器文件中创建，可参考框架内示例文件：process_test.php，有两种 

​      创建模式，分别为单独进程模式与进程池模式：**单独进程模式的进程间通信支持QUEUE消息队列；进程池模式**

​      **的进程间通信支持PIPE管道**，两种模式在config_swoole.php中配置切换。

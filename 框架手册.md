#####                        EASYLT框架采用MSVP设计模式，是MVC-MVP设计模式的升级版，在原有的model层中增加server层用于多进程、协程的TCP、UDP、HTTP、WEBSOCKET协议服务及RPC服务搭建，可快速实现业务RESTful、RPC的API及覆盖web、小程序、APP等开发的各种应用场景。使用前请仔细阅读以下文档；除标题外，加粗字体部分为重点需要注意的地方，否则框架运行会出错。
...
1.请参考model模型目录下的mysql_test.php文件中的代码进行创建新的model文件编写模型业务，所有实体类中可采用ORM（文档见下文）或原生写法（mysql连接变量为$mysql_conn）；如果采用原生mysqli写法，入参参数请用mysqli_real_escape_string()函数进行转义并在sql中对参数添加单引号，防止sql注入；

2.**$query_result为接口请求的入参值,数据类型为array，变量名不可更改，根据键值对取值；**

3.**$query_result['param']为请求URL的路由param值,用于判断同一model文件下多接口业务分发对接;**
...
1.index控制器为系统默认主控制器，无需再次创建控制器或修改，根据以下文档说明使用即可。

2.控制器路由示例：http://127.0.0.1/m/v/p/index/model/param/?key=value
...
- ?key=value为get请求入参，文件中用$_GET['key']取值，可选项；
...
- **model-server服务下创建的TCP、UDP、HTTP、WEBSOCKET、RPC服务器仅支持redis、mysqli的DB操作，不支持ORM；**
...
- 当进程启动时，通常用于长连接主动推送数据给客户端【可选项】
  ...
  //$mysql为mysql连接变量，可通过mysqli操作数据库，如mysqli_query($mysql,$sql);

  //$redis为redis连接变量，可操作redis，如$redis->set('username','easylt');

  //$mysql及$redis操作必须注册以上$websocket_serv->connect(function(){});并在其匿名回调函数中添加如下代码，否则报空指针：

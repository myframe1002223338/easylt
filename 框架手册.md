#####                        EASYLT框架采用全新的MSVP设计架构，全名EasyLite，是MVC/MVP设计架构的升级版，在原有的model层中增加server层，可实现业务代码内存常驻，用于多进程、协程的TCP、UDP、HTTP、WEBSOCKET协议服务及RPC服务搭建，可快速实现业务RESTful、RPC的API及覆盖web、小程序、APP等开发的各种应用场景。使用前请仔细阅读以下文档，除标题外，加粗字体部分为重点需要注意的地方，否则框架运行可能会出错。
...
- 当接收到请求数据时【必须实现】
  ...
  //如需返回给客户端数据请return $var，如在$websocket_serv->workerstart(function(){});的匿名函数中return $var返回给客户端数据，则这里不进行数据返回，如果没有业务实现调用该事件后方法体为空即可，receive、workerstart事件中仅可选择一个进行数据返回。
...
- 当进程启动时，通常用于长连接主动推送数据给客户端【可选项】
  ...
  **ps:请勿使用mysqli_close()，会导致无法持续从数据库中取数据推送给客户端。可以mysqli_free_result()释放结果集。**
...
- 协程可在TCP等服务器、子进程中创建，放置在匿名回调函数中，以下以TCP服务器为示例：
